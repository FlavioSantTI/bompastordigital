...
export default function DashboardPage() {
    ...
    // Estados Independentes para Lazy Loading (Frente 2)
    const [secGraficos, setSecGraficos] = useState({
        loaded: false,
        loading: false,
        confirmadas: 0,
        pendentes: 0,
    ...
    const [secInscPorEvento, setSecInscPorEvento] = useState<{
        loaded: boolean;
        loading: boolean;
        data: InscricaoPorEvento[];
    }>({ loaded: false, loading: false, data: [] });

    const [secPivot, setSecPivot] = useState<{
        loaded: boolean;
        loading: boolean;
        data: PivotRow[];
    }>({ loaded: false, loading: false, data: [] });

    const [secEventosRecentes, setSecEventosRecentes] = useState<{
        loaded: boolean;
        loading: boolean;
        data: EventoRecente[];
    }>({ loaded: false, loading: false, data: [] });

    const [secUltimasInscricoes, setSecUltimasInscricoes] = useState<{
        loaded: boolean;
        loading: boolean;
        data: any[];
    }>({ loaded: false, loading: false, data: [] });
    ...
    // 1. Carregar apenas estatísticas básicas na inicialização (Queries HEAD rápidas)
    const loadDashboardStats = useCallback(async () => {
        ...
        try {
            ...
            const [diocesesRes, eventosRes, inscricoesRes, pessoasRes, ativosRes] = await Promise.all([
                ...
                supabase.from('eventos')
                    ...
                    .eq('publicado', true)
                    .neq('status_manual', 'cancelado')
                    .gt('realizacao_fim', nowIso)
            ]);
            ...
            setStats({
                ...
                eventosAbertos: ativosRes.count || 0,
    ...
    // 2. Carregar gráficos de Status e Tipo sob demanda
    const loadGraficosData = async () => {
        if (secGraficos.loaded || secGraficos.loading) return;
        setSecGraficos(prev => ({ ...prev, loading: true }));
        try {
            ...
            const list = data || [];
            const confirmadas = list.filter(i => i.status === 'confirmada').length;
            const pendentes = list.filter(i => i.status !== 'confirmada').length;
            const casais = list.filter(i => i.tipo === 'casal').length;
            const individuais = list.filter(i => i.tipo === 'individual').length;
    ...
    // 3. Carregar Inscrições por Evento sob demanda
    const loadInscPorEventoData = async () => {
        if (secInscPorEvento.loaded || secInscPorEvento.loading) return;
        setSecInscPorEvento(prev => ({ ...prev, loading: true }));
        try {
            const { data: eventos } = await supabase
                .from('eventos')
                .select('id, nome, realizacao_inicio')
            ...
            if (eventos) {
                ...
                for (const evento of eventos) {
                    const evtInscricoes = list.filter(i => i.evento_id === evento.id);
            ...
            setError('Erro ao carregar inscrições por evento: ' + err.message);
        }
    };

    // 4. Carregar Pivot (Dioceses x Tipos) sob demanda
    const loadPivotData = async () => {
        if (secPivot.loaded || secPivot.loading) return;
        setSecPivot(prev => ({ ...prev, loading: true }));
        try {
            const { data: inscricoes } = await supabase
                .from('inscricoes')
                .select('diocese_id, tipo');
            ...
            setError('Erro ao carregar inscritos por diocese: ' + err.message);
        }
    };

    // 5. Carregar Eventos Recentes sob demanda
    const loadEventosRecentesData = async () => {
        if (secEventosRecentes.loaded || secEventosRecentes.loading) return;
        setSecEventosRecentes(prev => ({ ...prev, loading: true }));
        try {
            const { data: eventos } = await supabase
                .from('eventos')
                .select('id, nome, data_inicio, status_manual, vagas, realizacao_inicio')
                .order('realizacao_inicio', { ascending: false })
                .limit(5);
            ...
            if (eventos) {
                const eventosComContagem = await Promise.all(
                    eventos.map(async (evento) => {
                        ...
                        return {
                            ...
                            status: evento.status_manual || 'aberto',
                ...
                setSecEventosRecentes({
                    loaded: true,
                    loading: false,
                    data: eventosComContagem
                });
            }
        } catch (err: any) {
            setError('Erro ao carregar eventos recentes: ' + err.message);
        }
    };

    // 6. Carregar Últimas Inscrições sob demanda
    const loadUltimasInscricoesData = async () => {
        if (secUltimasInscricoes.loaded || secUltimasInscricoes.loading) return;
        setSecUltimasInscricoes(prev => ({ ...prev, loading: true }));
        try {
            ...
            if (inscricoesRecentes) {
                const inscricoesDetalhes = await Promise.all(
                    (inscricoesRecentes as any[]).map(async (insc) => {
                        ...
                        const { data: evento } = await supabase
                            .from('eventos')
                            .select('nome')
                            .eq('id', insc.evento_id ? Number(insc.evento_id) : 0)
                            .single();
                ...
                setSecUltimasInscricoes({
                    loaded: true,
                    loading: false,
                    data: inscricoesDetalhes
                });
            }
        } catch (err: any) {
            setError('Erro ao carregar últimas inscrições: ' + err.message);
        }
    };

    // 7. Atualizar dados globais (Recarrega apenas o que estava aberto/carregado)
    const handleGlobalRefresh = async () => {
        await loadDashboardStats();
        
        if (secGraficos.loaded) {
            setSecGraficos(prev => ({ ...prev, loaded: false }));
            setTimeout(loadGraficosData, 0);
        }
        if (secInscPorEvento.loaded) {
            setSecInscPorEvento(prev => ({ ...prev, loaded: false }));
            setTimeout(loadInscPorEventoData, 0);
        }
        if (secPivot.loaded) {
            setSecPivot(prev => ({ ...prev, loaded: false }));
            setTimeout(loadPivotData, 0);
        }
        if (secEventosRecentes.loaded) {
            setSecEventosRecentes(prev => ({ ...prev, loaded: false }));
            setTimeout(loadEventosRecentesData, 0);
        }
        if (secUltimasInscricoes.loaded) {
            setSecUltimasInscricoes(prev => ({ ...prev, loaded: false }));
            setTimeout(loadUltimasInscricoesData, 0);
        }
    };
    ...
    // Totais do pivot
    const pivotTotals = {
        casais: secPivot.data.reduce((acc, row) => acc + row.casais, 0),
        individuais: secPivot.data.reduce((acc, row) => acc + row.individuais, 0),
        total: secPivot.data.reduce((acc, row) => acc + row.total, 0),
    };
    ...
    const statusData = [
        { name: 'Confirmadas', value: secGraficos.confirmadas },
        { name: 'Pendentes', value: secGraficos.pendentes },
    ].filter(d => d.value > 0);

    const tipoData = [
        { name: 'Casais', value: secGraficos.casais },
        { name: 'Individuais', value: secGraficos.individuais },
    ].filter(d => d.value > 0);
    ...
    return (
        <Box>
            ...
            {/* ==================== ACORDIONS DE SEÇÕES PESADAS (Lazy Loading) ==================== */}

            {/* SEÇÃO 1: Gráficos de Pizza */}
            <Accordion 
                sx={{ mb: 2, borderRadius: 2, '&:before': { display: 'none' } }}
                onChange={(e, expanded) => expanded && loadGraficosData()}
            >
                <AccordionSummary expandIcon={<ExpandMore />}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                        <PieChartIcon color="primary" />
                        <Typography variant="h6" fontWeight="bold">
                            Gráficos de Inscrições e Tipos
                        </Typography>
                    </Box>
                </AccordionSummary>
                <AccordionDetails>
                    {secGraficos.loading ? (
                        ...
                        <Typography color="text.secondary">Clique para expandir e carregar os dados gráficos...</Typography>
                    ) : (
            ...
            {/* SEÇÃO 2: Inscrições por Evento */}
            <Accordion 
                sx={{ mb: 2, borderRadius: 2, '&:before': { display: 'none' } }}
                onChange={(e, expanded) => expanded && loadInscPorEventoData()}
            >
                <AccordionSummary expandIcon={<ExpandMore />}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                ...
                <AccordionDetails>
                    {secInscPorEvento.loading ? (
                        <Box sx={{ display: 'flex', justifyContent: 'center', p: 4 }}><CircularProgress /></Box>
                    ) : !secInscPorEvento.data || secInscPorEvento.data.length === 0 ? (
                        <Typography color="text.secondary">Nenhum evento com inscrições cadastradas...</Typography>
                    ) : (
                        <Paper
                            elevation={0}
                            sx={{
                        ...
                        >
                            ...
                            <ResponsiveContainer width="100%" height={Math.max(350, secInscPorEvento.data.length * 70)}>
                                <BarChart
                                    data={secInscPorEvento.data}
                                    layout="vertical"
                            ...
                            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', mt: 2, justifyContent: 'center' }}>
                                {secInscPorEvento.data.map((evt, i) => (
                                    <Box
                                        key={i}
                                        sx={{
                                            display: 'flex',
            ...
            {/* SEÇÃO 3: Inscritos por Diocese (Pivot Table) */}
            <Accordion 
                sx={{ mb: 2, borderRadius: 2, '&:before': { display: 'none' } }}
                onChange={(e, expanded) => expanded && loadPivotData()}
            >
                <AccordionSummary expandIcon={<ExpandMore />}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                ...
                <AccordionDetails>
                    {secPivot.loading ? (
                        <Box sx={{ display: 'flex', justifyContent: 'center', p: 4 }}><CircularProgress /></Box>
                    ) : !secPivot.loaded || secPivot.data.length === 0 ? (
                        <Typography color="text.secondary">Nenhuma diocese com inscrições cadastradas...</Typography>
                    ) : (
                        <Paper
                            sx={{
                                overflow: 'hidden',
                        ...
                        >
                            ...
                            <TableContainer sx={{ maxHeight: 520 }}>
                                <Table stickyHeader size="small">
                                    ...
                                    <TableBody>
                                        {secPivot.data.map((row, idx) => {
                                            const maxCasais = Math.max(...secPivot.data.map(r => r.casais));
                                            const maxIndiv = Math.max(...secPivot.data.map(r => r.individuais));
            ...
            <Accordion 
                sx={{ mb: 2, borderRadius: 2, '&:before': { display: 'none' } }}
                onChange={(e, expanded) => expanded && loadEventosRecentesData()}
            >
                <AccordionSummary expandIcon={<ExpandMore />}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                ...
                <AccordionDetails>
                    {secEventosRecentes.loading ? (
                        <Box sx={{ display: 'flex', justifyContent: 'center', p: 4 }}><CircularProgress /></Box>
                    ) : !secEventosRecentes.loaded || secEventosRecentes.data.length === 0 ? (
                        <Typography color="text.secondary">Nenhum evento recente cadastrado...</Typography>
                    ) : (
                        <Paper sx={{ p: 3, width: '100%' }}>
                            <TableContainer>
                                <Table>
                                    ...
                                    <TableBody>
                                        {secEventosRecentes.data.map((evento) => {
            ...
            {/* SEÇÃO 5: Últimas Inscrições */}
            <Accordion 
                sx={{ mb: 2, borderRadius: 2, '&:before': { display: 'none' } }}
                onChange={(e, expanded) => expanded && loadUltimasInscricoesData()}
            >
                <AccordionSummary expandIcon={<ExpandMore />}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                        <Group color="primary" />
                ...
                <AccordionDetails>
                    {secUltimasInscricoes.loading ? (
                        <Box sx={{ display: 'flex', justifyContent: 'center', p: 4 }}><CircularProgress /></Box>
                    ) : !secUltimasInscricoes.loaded || secUltimasInscricoes.data.length === 0 ? (
                        <Typography color="text.secondary">Nenhuma inscrição recente...</Typography>
                    ) : (
                        <TableContainer component={Paper} elevation={0} variant="outlined" sx={{ width: '100%' }}>
                            <Table size="small">
                                <TableHead>
                                    <TableRow>
                                ...
                                <TableBody>
                                    {secUltimasInscricoes.data.map((inscricao: any) => (
